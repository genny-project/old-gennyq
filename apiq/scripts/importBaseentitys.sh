...
if [ $# -eq 0 ]
then
   echo "usage: importBaseEntitys.sh <url> <username> <password> <sourceurl>"
   echo "e.g. ./importBaseEntitys.sh http://localhost:8096 dev1 password1 https://internmatch-test.gada.io"
   exit;
fi
...
echo $TOKEN
echo ""
echo "${url}/import/baseentitys ${sourceurl}"
CR=`curl -s -X GET "${url}/import/baseentitys?url=${sourceurl}"  --header "Authorization: Bearer $TOKEN" -H "accept: */*" -H "Content-Type: application/json"  --header 'Accept: application/json'  `
